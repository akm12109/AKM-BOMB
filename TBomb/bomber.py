...
def do_zip_update():
    ...
    print(ALL_COLORS[0]+"Downloading ZIP SUBSCRIBE akm* ... "+RESET_ALL)
    ...
    if success:
        mesgdcrt.SuccessMessage("AKM*-Bomb was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update AKM*-Bomb.")
...
def do_git_update():
    ...
    if success:
        ...
        print( WORKING NOW
            "git checkout . && "
            "git pull https://github.com/C.E.O_OF_AKM-TECH/AKM-BOMB HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("AKM*-Bomb is up-to-date")
        mesgdcrt.GeneralMessage("Starting AKM*-Bomb")
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("AKM*-Bomb was created by AKM*XTECH")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("AKM*-Bomb will work only in Python v3")
    sys.exit()
...
description = """AKM*-Bomb - SPAM FREINDLY FOR SCAM !

AKM-Bomb can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

AKM-Bomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start AKM-Bomb with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start AKM-Bomb with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start AKM-Bomb with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update AKM-Bomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current AKM-Bomb contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current AKM-Bomb version")
